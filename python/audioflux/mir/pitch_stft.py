...
class OpaquePitchSTFT(Structure):
    _fields_ = []


class PitchSTFT(Base):
    """
    ...
    def __init__(self, samplate=32000, low_fre=32.0, high_fre=2000.0,
        ...
        super(PitchSTFT, self).__init__(pointer(OpaquePitchSTFT()))
        ...
        fn = self._lib['pitchSTFTObj_new']
        fn.argtypes = [POINTER(POINTER(OpaquePitchSTFT)),
    ...
    def cal_time_length(self, data_length):
        ...
        out: int
        """
        fn = self._lib['pitchSTFTObj_calTimeLength']
        fn.argtypes = [POINTER(OpaquePitchSTFT), c_int]
        fn.restype = c_int
    ...
    def pitch(self, data_arr):
        ...
        fn = self._lib['pitchSTFTObj_pitch']
        fn.argtypes = [POINTER(OpaquePitchSTFT),
        ...
        if data_arr.ndim == 1:
            ...
            db_arr = np.zeros(size, dtype=np.float32)
            for i in range(channel_num):
                fn(self._obj, data_arr[i], c_int(data_len), fre_arr[i], db_arr[i])
    ...
    def __del__(self):
        if self._is_created:
            fn = self._lib['pitchSTFTObj_free']
            fn.argtypes = [POINTER(OpaquePitchSTFT)]
            fn.restype = c_void_p
            fn(self._obj)
