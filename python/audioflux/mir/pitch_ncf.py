...
class OpaquePitchNCF(Structure):
    _fields_ = []


class PitchNCF(Base):
    """
    Normalized Correlation Function(NCF). Pitch is estimated using normalized time-domain autocorrelation. [#]_

    .. [#] Atal, B.S. "Automatic Speaker Recognition Based on Pitch Contours." The Journal of the Acoustical
           Society of America. Vol. 52, No. 6B, 1972, pp. 1687–1697.`
    ...
    def __init__(self, samplate=32000, low_fre=32.0, high_fre=2000.0,
                 radix2_exp=12, slide_length=1024, window_type=WindowType.RECT):
        super(PitchNCF, self).__init__(pointer(OpaquePitchNCF()))
        ...
        fn = self._lib['pitchNCFObj_new']
        fn.argtypes = [POINTER(POINTER(OpaquePitchNCF)),
    ...
    def cal_time_length(self, data_length):
        ...
        out: int
        """
        fn = self._lib['pitchNCFObj_calTimeLength']
        fn.argtypes = [POINTER(OpaquePitchNCF), c_int]
        fn.restype = c_int
    ...
    def pitch(self, data_arr):
        ...
        fn = self._lib['pitchNCFObj_pitch']
        fn.argtypes = [POINTER(OpaquePitchNCF),
